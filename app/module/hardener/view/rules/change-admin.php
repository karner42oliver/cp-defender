<div class="rule closed" id="change_admin">
    <div class="rule-title">
		...
		<?php endif; ?>
		<?php _e( "Change default admin user account", cp_defender()->domain ) ?>
    </div>
    <div class="rule-content">
        ...
        <div class="line end">
            ...
            fixed. Ensure you backup your database before choosing a new username.", cp_defender()->domain ) ?>
        </div>
        <h3>
        ...
        <div class="well has-input">
			<?php if ( $controller->check() ): ?>
				<?php _e( "You don’t have a user with username admin.", cp_defender()->domain ) ?>
			<?php else: ?>
                <div class="line">
                    <p><?php _e( "Please change the username from admin to something unique.", cp_defender()->domain ) ?></p>
                </div>
                <form method="post" class="hardener-frm rule-process">
					<?php $controller->createNonceField(); ?>
                    <input type="hidden" name="action" value="processHardener"/>
                    <input type="text" placeholder="<?php esc_attr_e( "Enter new username", cp_defender()->domain ) ?>"
