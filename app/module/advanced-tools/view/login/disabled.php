                ...
                <div id="def2">
                    <div class="destroy-sessions">
                        <button type="button" class="button" id="show2AuthActivator">
							<?php _e( "Enable", cp_defender()->domain ) ?>
                        </button>
                    </div>
                    <p class="description">
						<?php _e( "Use the Google Authenticator app to sign in with a separate passcode.", cp_defender()->domain ) ?>
                    </p>
                </div>
                <div id="def2qr">
                    <button type="button" id="hide2AuthActivator"
                            class="button"><?php _e( "Cancel", cp_defender()->domain ) ?></button>
                    <p><?php _e( "Use the Google Authenticator app to sign in with a separate passcode.", cp_defender()->domain ) ?></p>
                    <div class="card">
                        <p>
                            <strong><?php _e( "1. Install the Verification app", cp_defender()->domain ) ?></strong>
                        </p>
                        <p>
							<?php _e( "Download and install the Google Authenticator app on your device using the links below.", cp_defender()->domain ) ?>
                        </p>
                        ...
                        <div class="line"></div>
                        <p><strong><?php _e( "2. Scan the barcode", cp_defender()->domain ) ?></strong></p>
                        <p><?php _e( "Open the Google Authenticator app you just downloaded, tap the “+” symbol and then use your phone’s camera to scan the barcode below.", cp_defender()->domain ) ?></p>
                        <img class="barcode"
                        ...
                        <div class="line"></div>
                        <p><strong><?php _e( "3. Enter passcode", cp_defender()->domain ) ?></strong></p>
                        <p>
							<?php _e( "Enter the 6 digit passcode that is shown on your device into the input field below and hit “Verify”.", cp_defender()->domain ) ?>
                        </p>
                        <div class="well">
                            <p class="error"></p>
                            <input type="text" id="otpCode" class="def-small-text">
                            <button type="button" class="button button-primary" id="verifyOTP">
								<?php _e( "Verify", cp_defender()->domain ) ?>
                            </button>
