...
namespace CP_Defender\Module\IP_Lockout\Component;
...
class Login_Protection_Api extends Component {
	...
	public static function getLogsActionsText( $log ) {
		...
		if ( ! in_array( $log->ip, $whitelist ) ) {
			$links[] = '<a data-nonce="' . $nonce . '" class="ip-action" data-type="whitelist" data-id="' . $log->id . '" href="#">' . __( "Whitelist", cp_defender()->domain ) . '</a>';
		} else {
			$links[] = '<a data-nonce="' . $nonce . '" class="ip-action" data-type="unwhitelist" data-id="' . $log->id . '" href="#">' . __( "Unwhitelist", cp_defender()->domain ) . '</a>';
		}
