...
use CP_Defender\Controller;
use CP_Defender\Module\Audit\Component\Audit_API;
use CP_Defender\Module\IP_Lockout\Behavior\IP_Lockout;
use CP_Defender\Module\IP_Lockout\Component\Login_Protection_Api;
use CP_Defender\Module\IP_Lockout\Component\Logs_Table;
use CP_Defender\Module\IP_Lockout\Model\IP_Model;
use CP_Defender\Module\IP_Lockout\Model\IP_Model_Legacy;
use CP_Defender\Module\IP_Lockout\Model\Log_Model;
use CP_Defender\Module\IP_Lockout\Model\Log_Model_Legacy;
use CP_Defender\Module\IP_Lockout\Model\Settings;
...
// Wenn netzwerkweit aktiviert, nur im Netzwerk-Admin anzeigen
if ( $this->is_network_activate( cp_defender()->plugin_slug ) && ! is_network_admin() ) {
return;
}
	...
	public function scripts() {
		if ( $this->isInPage() ) {
			...
			wp_enqueue_script( 'iplockout', cp_defender()->getPluginUrl() . 'app/module/ip-lockout/js/script.js' );
		} else {
			wp_enqueue_script( 'iplockout', cp_defender()->getPluginUrl() . 'app/module/ip-lockout/js/script.js' );
		}
	}
