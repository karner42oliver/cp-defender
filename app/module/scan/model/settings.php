...
namespace CP_Defender\Module\Scan\Model;

use Hammer\Helper\Log_Helper;
use Hammer\Helper\WP_Helper;
use Hammer\Queue\Queue;
use CP_Defender\Module\Scan\Behavior\Core_Scan;
use CP_Defender\Module\Scan\Behavior\Pro\Content_Scan;
use CP_Defender\Module\Scan\Behavior\Pro\Vuln_Scan;
...
Official PSOURCE Superhero', cp_defender()->domain );
	...
	public static function instance() {
		if ( is_null( self::$_instance ) ) {
			$class           = new Settings( 'wd_scan_settings', WP_Helper::is_network_activate( cp_defender()->plugin_slug ) );
	...
	public static function queueFactory( $slug, $args = array() ) {
		switch ( $slug ) {
			case 'core':
				...
				return $queue;
			case 'vuln':
				if ( ! class_exists( '\CP_Defender\Module\Scan\Behavior\Pro\Vuln_Scan' ) ) {
					return null;
				}
				...
				break;
			case 'content':
				if ( ! class_exists( '\CP_Defender\Module\Scan\Behavior\Pro\Content_Scan' ) ) {
					return null;
				}
