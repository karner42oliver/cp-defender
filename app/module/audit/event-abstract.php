...
namespace CP_Defender\Module\Audit;
...
abstract class Event_Abstract extends \Hammer\WP\Component {
	...
	private function get_text( $text, $params ) {
		...
		foreach ( $text as $row ) {
			...
			if ( is_array( $value ) && is_array( $compare ) ) {
				//comare 2 array
				$map = array(
					'==' => count( self::arrayRecursiveDiff( $value, $compare ) ) == 0 ? true : false,
					'!=' => count( self::arrayRecursiveDiff( $value, $compare ) ) == 0 ? false : true
				);
	...
	public function build_log_data() {
		...
		if ( isset( $hook_data['callback'] ) && ! empty( $hook_data['callback'] ) ) {
			...
			$ret               = $reflection_method->invokeArgs( new Audit_API(), array(
				$hook_name,
				$params
			) );
			...
			if ( isset( $hook_data['context'] ) ) {
				if ( is_callable( $hook_data['context'] ) ) {
					...
					$context           = $reflection_method->invokeArgs( new Audit_API(), array(
						$hook_name,
						$params
					) );
