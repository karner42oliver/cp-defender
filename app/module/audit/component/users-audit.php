		...
		return array(
			...
			'user_register'         => array(
				...
				'program_args' => array(
					...
					'user_role' => array(
						'callable' => array( '\CP_Defender\Module\Audit\Component\Users_Audit', 'get_user_role' ),
						'params'   => array(
							'{{user_id}}'
						),
					)
			...
			'profile_update'        => array(
				...
				'callback'    => array( '\CP_Defender\Module\Audit\Component\Users_Audit', 'profile_update_callback' ),
			),
			...
			/*'updated_user_meta1'    => array(
				...
				'text'         => sprintf( esc_html__( "%s changed user %s meta %s", cp_defender()->domain ), '{{wp_user}}', '{{username}}', '{{meta_key}}' ),
				'action_type'  => Audit_API::ACTION_UPDATED,
				'context'      => self::CONTEXT_PROFILE,
				'program_args' => array(
					'username' => array(
