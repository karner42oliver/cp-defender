...
class Comment_Audit extends Event_Abstract {
	...
	public function get_hooks() {
		return array(
			'wp_insert_comment'         => array(
				...
				'callback'    => array( '\CP_Defender\Module\Audit\Component\Comment_Audit', 'processGenericComment' )
			),
			'comment_flood_trigger'     => array(
				...
				'text'        => sprintf( esc_html__( "User %s flooded comment", cp_defender()->domain ), '{{wp_user}}' ),
			),
			...
			'transition_comment_status' => array(
				...
				'callback'    => array( '\CP_Defender\Module\Audit\Component\Comment_Audit', 'processCommentStatusChanged' ),
			),
			'edit_comment'              => array(
				'args'        => array( 'comment_ID' ),
				'level'       => self::LOG_LEVEL_INFO,
			...
			'comment_duplicate_trigger' => array(
				...
				'callback'    => array( '\CP_Defender\Module\Audit\Component\Comment_Audit', 'processCommentDuplicateTrigger' ),
			),
		);
	}
	...
	public function dictionary() {
		return array(
			self::ACTION_DUPLICATED => esc_html__( "Duplicated", cp_defender()->domain ),
			self::ACTION_SPAMMED    => esc_html__( "Spammed", cp_defender()->domain ),
			self::ACTION_UNSPAMMED  => esc_html__( "Unspammed", cp_defender()->domain ),
			self::CONTEXT_COMMENT   => esc_html__( "Comment", cp_defender()->domain )
		);
	}
		...
		switch ( $hookname ) {
			case 'comment_post':
				if ( $comment_approved === 'spam' ) {
					...
					$comment_status = esc_html__( "approved", cp_defender()->domain );
				} else {
					$comment_status = esc_html__( "pending approval", cp_defender()->domain );
				}
				if ( $comment['comment_parent'] == 0 ) {
					$text = sprintf( esc_html__( "%s commented on %s \"%s\" - comment status: %s", cp_defender()->domain ),
						$comment['comment_author'], $post_type_label, $post->post_title, $comment_status );
				} else {
					$parent_comment = get_comment( $comment['comment_parent'] );
					$text           = sprintf( esc_html__( "%s replied to %s's comment on %s \"%s\" - comment status: %s", cp_defender()->domain ),
