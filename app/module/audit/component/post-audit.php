...
class Post_Audit extends Event_Abstract {
	...
	public function cache_post_updated( $post_id, $after, $before ) {
		cp_defender()->global['post_updated'][] = array(
			'post_id' => $post_id,
			'after'   => $after,
			'before'  => $before
		);
	...
	public function get_hooks() {
		return array(
			'save_post'              => array(
				...
				'callback'    => array( '\CP_Defender\Module\Audit\Component\Post_Audit', 'post_updated_callback' ),
				'event_type'  => 'content',
	...
	public static function post_updated_callback() {
		...
		$post_before = null;
		if ( isset( cp_defender()->global['post_updated'] ) && is_array( cp_defender()->global['post_updated'] ) ) {
			foreach ( cp_defender()->global['post_updated'] as $post_arr ) {
