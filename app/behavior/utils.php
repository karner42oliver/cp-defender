...
use CP_Defender\Module\IP_Lockout\Component\Login_Protection_Api;
...
class Utils extends Behavior {
	...
	public function isActivatedSingle() {
		if ( WP_Helper::is_network_activate( cp_defender()->plugin_slug ) ) {
			return false;
		}
	...
	public function allowedHtml() {
		return array(
			...
			'img'    => array(
				'class' => 'text-warning',
				'src'   => cp_defender()->getPluginUrl() . 'assets/img/robot.png'
			)
		);
	}
	...
	public function determineServer( $useStaticPath = false ) {
		$url         = ( $useStaticPath ) ? cp_defender()->getPluginUrl() . 'changelog.txt' : home_url();
	...
	public function getDefUploadDir() {
		...
		$defDir    = $uploadDir . DIRECTORY_SEPARATOR . 'cp-defender';
		if ( ! is_dir( $defDir ) ) {
			wp_mkdir_p( $defDir );
		}
	...
	public function generateStats() {
		...
		foreach ( $issue_rules as $rule ) {
			$issues[] = array(
				'label' => $rule->getTitle(),
				'url'   => $this->getAdminPageUrl( 'wdf-hardener' ) . '#' . $rule::$slug
			);
		}
		foreach ( $ignored_rules as $rule ) {
			$ignored[] = array(
				'label' => $rule->getTitle(),
				'url'   => $this->getAdminPageUrl( 'wdf-hardener', array( 'view' => 'ignored' ) ) . '#' . $rule::$slug
			);
		}
		foreach ( $fixed_rules as $rule ) {
			$resolved[] = array(
				'label' => $rule->getTitle(),
				'url'   => $this->getAdminPageUrl( 'wdf-hardener', array( 'view' => 'resolved' ) ) . '#' . $rule::$slug
			);
		}
		...
		$scanSettings    = \CP_Defender\Module\Scan\Model\Settings::instance();
		$after_time      = '';
		...
		$events_in_month = \CP_Defender\Module\Audit\Component\Audit_API::pullLogs( array(
		...
		$data = array(
			...
			'scan_data'    => json_encode(
				array(
					...
					'new_scan_url'          => $this->getAdminPageUrl( 'wdf-scan', array( 'wdf-action' => 'new_scan' ) ),
					'schedule_scans_url'    => $this->getAdminPageUrl( 'wdf-schedule-scan' ),
					'settings_page_url'     => $this->getAdminPageUrl( 'wdf-settings' ),
					'ip_lockout_page_url'   => $this->getAdminPageUrl( 'wdf-ip-lockout' ),
					...
					'reports'               => array(
						'file_scanning' => array(
							'active'    => true,
							'enabled'   => \CP_Defender\Module\Scan\Model\Settings::instance()->notification,
							//Report enabled Bool
							'frequency' => array(
								'frequency' => \CP_Defender\Module\Scan\Model\Settings::instance()->frequency,
								'day'       => \CP_Defender\Module\Scan\Model\Settings::instance()->day,
								'time'      => \CP_Defender\Module\Scan\Model\Settings::instance()->time
							)
						),
						'audit_logging' => array(
							'active'    => \CP_Defender\Module\Audit\Model\Settings::instance()->enabled,
							'enabled'   => \CP_Defender\Module\Audit\Model\Settings::instance()->notification,
							'frequency' => array(
								'frequency' => \CP_Defender\Module\Audit\Model\Settings::instance()->frequency,
								'day'       => \CP_Defender\Module\Audit\Model\Settings::instance()->day,
								'time'      => \CP_Defender\Module\Audit\Model\Settings::instance()->time
							)
						),
						'ip_lockouts'   => array(
							//always true as we have blacklist listening
							'active'    => true,
							'enabled'   => \CP_Defender\Module\IP_Lockout\Model\Settings::instance()->report,
							//Report enabled Bool
							'frequency' => array(
								'frequency' => \CP_Defender\Module\IP_Lockout\Model\Settings::instance()->report_frequency,
								'day'       => \CP_Defender\Module\IP_Lockout\Model\Settings::instance()->report_day,
								'time'      => \CP_Defender\Module\IP_Lockout\Model\Settings::instance()->report_time
							),
						)
					),
				)
	...
	/**
	 * Generiere die korrekte Admin-URL basierend auf Netzwerk-Aktivierung
	 * @param string $page Seiten-Slug
	 * @param array $args Zusätzliche Query-Parameter
	 * @return string
	 */
	public function getAdminPageUrl( $page, $args = array() ) {
		...
		if ( WP_Helper::is_network_activate( cp_defender()->plugin_slug ) ) {
			return network_admin_url( 'admin.php?' . $query_string );
		} else {
