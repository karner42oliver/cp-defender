...
class Email_Search extends Component {
	...
	public function renderInput() {
		if ( empty( $this->placeholder ) ) {
			$this->placeholder = __( "Enter a username", cp_defender()->domain );
		}
		?>
            ...
            <div class="receipt">
						...
						<?php if ( is_object( $user ) ): ?>
                            <li><?php echo get_avatar( $user->ID, 30 ) ?>
								...
								<?php if ( get_current_user_id() == $user->ID ): ?>
                                    <span class="def-tag tag-generic"><?php esc_html_e( "You", cp_defender()->domain ) ?></span>
								<?php endif; ?>
                                <a data-id="<?php echo esc_attr( $user->ID ) ?>"
                                   class="remove wd-remove-recipient float-r"
                                   href="#"><?php esc_html_e( "Remove", cp_defender()->domain ) ?></a>
                            </li>
						<?php endif; ?>
					<?php endforeach; ?>
                </ul>
                <div>
                    ...
                    <button type="submit" disabled id="add-receipt"
                            class="button button-grey"><?php _e( "Add", cp_defender()->domain ) ?></button>
                </div>
            </div>
		<?php else: ?>
                ...
                $('#add-receipt').click(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.is_current) {
                                user_row.append($('<span/>').addClass('def-tag tag-generic').html('<?php esc_html_e( "You", cp_defender()->domain ) ?>'))
                            }
                            user_row.append($('<a/>').attr({
                                'data-id': data.user_id,
                                'class': 'remove float-r wd-remove-recipient',
                                'href': '#'
                            }).html('<?php esc_html_e( "Remove", cp_defender()->domain ) ?>'))
